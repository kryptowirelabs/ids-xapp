...
static
void log_int_value(char* const meas_name, meas_record_lst_t meas_record)
{
  if (strncmp(meas_name, "RRU.PrbTotDl", 12) == 0 || strncmp(meas_name, "RRU.PrbTotUl", 12) == 0) {
    printf("%s = %d [PRBs]\n", meas_name, meas_record.int_val);
  } else if (strncmp(meas_name, "DRB.PdcpSduVolumeDL", 19) == 0 || strncmp(meas_name, "DRB.PdcpSduVolumeUL", 19) == 0) {
    printf("%s = %d [kb]\n", meas_name, meas_record.int_val);
  } else {
...
static
void log_real_value(char* const meas_name, meas_record_lst_t meas_record)
{
  if (strncmp(meas_name, "DRB.RlcSduDelayDl", 17) == 0) {
    printf("%s = %.2f [μs]\n", meas_name, meas_record.real_val);
  } else if (strncmp(meas_name, "DRB.UEThpDl", 11) == 0 || strncmp(meas_name, "DRB.UEThpUl", 11) == 0) {
    printf("%s = %.2f [kbps]\n", meas_name, meas_record.real_val);
  } else {
    printf("Measurement Name not yet supported %s\n", meas_name);
  }
...
typedef void (*log_meas_value)(char* const meas_name, meas_record_lst_t meas_record);
...
void match_meas_name_type(meas_type_t meas_type, meas_record_lst_t meas_record)
{
  ...
  char* const meas_name = cp_ba_to_str(meas_type.name);
...
void match_id_meas_type(meas_type_t meas_type, meas_record_lst_t meas_record)
{
  (void)meas_type;
  (void)meas_record;
  assert(false && "ID Measuement Name not yet supported");
}
                             ...
                             e2sm_rc_ctrl_msg_frmt_1_t* msg)
{
  ...
  for (size_t i = 0; i < sz; i++) {
    ...
    assert(strncmp(control_act_name, "QoS flow mapping configuration", 30) == 0 && "Add requested CONTROL Action. At the moment, only QoS flow mapping configuration supported");
...
rc_ctrl_req_data_t gen_rc_ctrl_msg(ran_func_def_ctrl_t const* src)
{
  ...
  for (size_t i = 0; i < src->sz_seq_ctrl_style; i++) {
    ...
    assert(strncmp(control_name, "Radio Bearer Control", 20) == 0 && "Add requested CONTROL Style. At the moment, only Radio Bearer Control supported");
