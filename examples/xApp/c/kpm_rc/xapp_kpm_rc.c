                             ...
                             e2sm_rc_ctrl_msg_frmt_1_t* msg)
{
  ...
  for (size_t i = 0; i < sz; i++) {
    char* control_act_name = cp_ba_to_str(ctrl_act[i].name);
    if (strcmp(control_act_name, "QoS flow mapping configuration") == 0) {
      ...
      msg->ran_param[0].ran_param_id = DRB_ID_8_4_2_2;
      msg->ran_param[0].ran_param_val.type = ELEMENT_KEY_FLAG_TRUE_RAN_PARAMETER_VAL_TYPE;
      msg->ran_param[0].ran_param_val.flag_true = calloc(1, sizeof(ran_parameter_value_t));
      assert(msg->ran_param[0].ran_param_val.flag_true != NULL && "Memory exhausted");
      // Let's suppose that it is the DRB 5
      msg->ran_param[0].ran_param_val.flag_true->type = INTEGER_RAN_PARAMETER_VALUE;
      msg->ran_param[0].ran_param_val.flag_true->int_ran = 5;

      // List of QoS Flows to be modified in DRB
      assert(ctrl_act[i].assoc_ran_param[1].id == LIST_OF_QOS_FLOWS_MOD_IN_DRB_8_4_2_2);
      msg->ran_param[1].ran_param_id = LIST_OF_QOS_FLOWS_MOD_IN_DRB_8_4_2_2;
      msg->ran_param[1].ran_param_val.type = LIST_RAN_PARAMETER_VAL_TYPE;
      msg->ran_param[1].ran_param_val.lst = calloc(1, sizeof(ran_param_list_t));
      assert(msg->ran_param[1].ran_param_val.lst != NULL && "Memory exhausted");
      ran_param_list_t* rpl = msg->ran_param[1].ran_param_val.lst;
      ...
      assert(false && "Add requested CONTROL Action. At the moment, only QoS flow mapping configuration supported");
    }
    free(control_act_name);
  }
}

static
rc_ctrl_req_data_t gen_rc_ctrl_msg(ran_func_def_ctrl_t const* src)
{
  assert(src != NULL);
  ...
  for (size_t i = 0; i < src->sz_seq_ctrl_style; i++) {
    char* control_name = cp_ba_to_str(src->seq_ctrl_style[i].name);
    if (strcmp(control_name, "Radio Bearer Control") == 0) {
      // CONTROL HEADER
      rc_ctrl.hdr.format = src->seq_ctrl_style[i].hdr;
      ...
      // CONTROL MESSAGE
      rc_ctrl.msg.format = src->seq_ctrl_style[i].msg;
      assert(rc_ctrl.msg.format == FORMAT_1_E2SM_RC_CTRL_MSG && "Indication Message Format received not valid");

      fill_rc_ctrl_act(src->seq_ctrl_style[i].seq_ctrl_act,
                       src->seq_ctrl_style[i].sz_seq_ctrl_act,
                       &rc_ctrl.hdr.frmt_1,
                       &rc_ctrl.msg.frmt_1);

    } else {
      assert(false && "Add requested CONTROL Style. At the moment, only Radio Bearer Control supported");
    }
    free(control_name);
  }
...
int main(int argc, char* argv[])
{
  ...
  for (size_t i = 0; i < nodes.len; ++i) {
    ...
    size_t idx = find_sm_idx(n->rf, n->len_rf, eq_sm, RC_ran_function);
