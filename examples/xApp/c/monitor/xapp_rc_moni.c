...
void log_element_ran_param_value(ran_parameter_value_t* param_value)
{
  ...
  switch (param_value->type) {
    ...
    default:
      assert(false && "Add corresponding print function for the RAN Parameter Value (other than integer)\n");
  }
}
...
void log_ran_param_name(uint32_t id)
{
  switch (id) {
    ...
    default:
      assert(false && "Add corresponding RAN Parameter ID\n");
  }
}
    ...
    for (size_t i = 0; i < ind_msg_frmt_2->sz_seq_ue_id; i++) {
      ...
      for (size_t j = 0; j < ue_id_item->sz_seq_ran_param; j++) {
        ...
        switch (ran_param_item->ran_param_val.type) {
          ...
          default:
            assert(false && "Add corresponding function for the RAN Parameter Value Type (other than element)\n");
        }
      }
    }
...
rc_sub_data_t gen_rc_sub_msg(ran_func_def_report_t const* src)
{
  ...
  for (size_t i = 0; i < src->sz_seq_report_sty; i++) {
    ...
    assert(strncmp(report_name, "UE Information", 14) == 0 && "Add requested REPORT Style. At the moment, only UE Information supported");
    ...
    for (size_t j = 0; j < sz; j++) {
      ...
      assert(strncmp(ran_param_name, "RRC State", 9) == 0 && "Add requested RAN Parameter. At the moment, only RRC State supported");
