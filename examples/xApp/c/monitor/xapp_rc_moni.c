...
// Print integer value
static
void print_int_ran_param_value(int64_t value)
{
...
static
void print_element_ran_param_value(ran_parameter_value_t* param_value)
{
...
static
void print_ran_param_name(uint32_t id)
{
  switch (id) {
...
static
void print_gnb_id_e2sm(gnb_e2sm_t* gnb)
{
  assert(gnb != NULL);
...
static
rc_sub_data_t gen_rc_sub_msg(ran_func_def_report_t const* src)
{
  assert(src != NULL);
  ...
  for (size_t i = 0; i < src->sz_seq_report_sty; i++) {
    char* report_name = cp_ba_to_str(src->seq_report_sty[i].name);
    if (strcmp(report_name, "UE Information") == 0) { // as defined in section 7.4.5, formats used for SUBSCRIPTION msg are known
      size_t const sz = src->seq_report_sty[i].sz_seq_ran_param;

      // Generate Event Trigger
      rc_sub.et.format = src->seq_report_sty[i].ev_trig_type;
      ...
      rc_sub.ad[0].format = src->seq_report_sty[i].act_frmt_type;
      ...
      for (size_t j = 0; j < sz; j++) {
        char* ran_param_name = cp_ba_to_str(src->seq_report_sty[i].ran_param[j].name);
        if (strcmp(ran_param_name, "RRC State") == 0) {
          ue_info_chng_trigger_type_e const trigger_type = RRC_STATE_UE_INFO_CHNG_TRIGGER_TYPE;
          uint32_t const ran_param_id = src->seq_report_sty[i].ran_param[j].id;
          ran_param_def_t const* ran_param_def = src->seq_report_sty[i].ran_param[j].def;
          // Fill Event Trigger
          ...
          assert(false && "Add requested RAN Parameter. At the moment, only RRC State supported");
        }
        free(ran_param_name);
      }
    } else {
